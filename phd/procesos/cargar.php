...
// if everything is ok, try to upload file
	...
	$idPHD=$_POST['idPHDB'];
	$delete = mysqli_query($conexion, "SELECT adjuntos FROM phd WHERE idPHD='$idPHD'");
	while($ver=mysqli_fetch_row($delete)){
	unlink($ver[0]);	
	$insert = mysqli_query($conexion,"UPDATE phd SET adjuntos = '$uploadedFile' WHERE idPHD = '$idPHD'");
