...
// if everything is ok, try to upload file
	...
	$idPHD=$_POST['idImagenesB'];
	$delete = mysqli_query($conexion, "SELECT adjuntos FROM radiologia WHERE idRadiologia='$idPHD'");
	while($ver=mysqli_fetch_row($delete)){
	unlink($ver[0]);	
	$insert = mysqli_query($conexion,"UPDATE radiologia SET adjuntos = '$uploadedFile' WHERE idRadiologia = '$idPHD'");
