...
// if everything is ok, try to upload file
	...
	$idPHD=$_POST['idContrareferenciaB'];
	$delete = mysqli_query($conexion, "SELECT adjuntos FROM contrareferencia WHERE idContrareferencia='$idPHD'");
	while($ver=mysqli_fetch_row($delete)){
	unlink($ver[0]);	
	$insert = mysqli_query($conexion,"UPDATE contrareferencia SET adjuntos = '$uploadedFile' WHERE idContrareferencia = '$idPHD'");
